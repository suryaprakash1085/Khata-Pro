...
import { sendOtpSms, verifyOtpSms, SmsError } from "../services/sms";
...
router.post("/auth/send-otp", async (req, res): Promise<void> => {
  ...
  try {
    ...
    req.log.error({ err, phone: normalizedPhone }, "Failed to send OTP");
    res.status(502).json({ error: "Failed to send OTP" });
    return;
  }

  req.log.info({ phone: normalizedPhone }, "OTP requested");
  res.json({ message: "OTP sent successfully" });
});
...
router.post("/auth/verify-otp", async (req, res): Promise<void> => {
  ...
  const isValid = await verifyOtpSms(normalizedPhone, otp);
  if (!isValid) {
    res.status(400).json({ error: "Invalid OTP" });
    return;
  }
