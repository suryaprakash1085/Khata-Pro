...
// //   // Stats must be based on each customer's NET balance (current_balance),
// //   // not raw sums of you_gave / you_got transaction amounts. A single
// //   // settled bill (you_gave 389 + you_got 389) nets to zero and should not
// //   // show up as "389 to collect AND 389 to pay" — that double counts every
// //   // settled sale. current_balance already reflects the running net per
// //   // customer, so we just bucket those into collect vs pay.
// //   const customerBalances = await db.select({
// //     currentBalance: customersTable.currentBalance,
// //   }).from(customersTable)
// //     .where(and(eq(customersTable.businessId, id), eq(customersTable.isDeleted, false)));

// //   let totalToCollect = 0; // customers who owe the business money (positive balance)
// //   let totalToPay = 0; // customers/suppliers the business owes money to (negative balance)
...
//   // "You'll get" = gross sales total (every bill ever billed to a customer —
//   // paid or on credit, doesn't matter). This is the sum of all you_gave
//   // amounts, since every bill is recorded as a you_gave transaction
//   // regardless of whether it was settled immediately.
...
//   // Today's sales — same "you_gave" logic, filtered to transactions whose
//   // entry_date is today. entry_date is a plain `date` column (no timezone),
//   // so a direct equality check against current_date is correct.
...
//   // "You'll pay" = what the business owes suppliers/customers — this stays
//   // based on each customer's NET balance (current_balance), since it
//   // represents an actual outstanding debt, not a gross total.
//   const customerBalances = await db.select({
//     currentBalance: customersTable.currentBalance,
//   }).from(customersTable)
//     .where(and(eq(customersTable.businessId, id), eq(customersTable.isDeleted, false)));
