...
//   if (search) {
//     conditions.push(
//       or(ilike(productsTable.name, `%${search}%`), ilike(productsTable.barcode, `%${search}%`), ilike(productsTable.sku, `%${search}%`)),
//     );
//   }
...
// // GET /products/barcode/:barcode  (must come before /products/:id to avoid route collision)
...
import { 
  ...
  lte, 
  inArray, 
  notInArray // ✅ 1. Added missing import here
} from "drizzle-orm";
import { requireAuth } from "../middlewares/auth";
import {
  CreateProductBody,
...
// ✅ 2. NEW ROUTE: GET /public/products/suggestions
