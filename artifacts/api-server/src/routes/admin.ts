...
router.get("/admin/analytics", requireAdmin, async (req, res): Promise<void> => {
  try {
    ...
    const monthlyGrowth = await Promise.all(
      monthlyData.map(
        async ({ month, startOfMonth, endOfMonth }) => {
          const [bizCount, userCount, txCount] =
            await Promise.all([
              db
                .select({ count: count() })
                .from(businessesTable)
                .where(
                  sql`created_at::date between ${startOfMonth} and ${endOfMonth}`
                ),
...
>>>>>>> 54571b9db09ab889e729432cc5d0441746689f17
    ...
    res.json({
      ...
      total_transaction_volume: parseFloat(
        txVolume[0].total ?? "0"
      ),
...
router.post(
  ...
  async (req, res): Promise<void> => {
    const parsed =
      BroadcastNotificationBody.safeParse(
        req.body
      );
