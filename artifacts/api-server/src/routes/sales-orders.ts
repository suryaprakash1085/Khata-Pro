...
function formatSalesOrder(so: any, customerName?: string, itemCount?: number, paymentMode?: string | null) {
  return {
    ...
    // payment mode (e.g. 'online' | 'cod' | 'card'), sourced from the linked
    // deliveries row. null for in-store pickup orders that never got a
    // deliveries row.
...
// GET /sales-orders
// Also joins deliveries to resolve each order's payment_mode.
router.get("/sales-orders", requireAuth, async (req, res): Promise<void> => {
  ...
  const [customers, itemCounts, deliveries] = await Promise.all([
    ...
    orderIds.length > 0
      ...
      : Promise.resolve([] as any[]),
    // Fetch payment_method per sales order via the linked deliveries row
    orderIds.length > 0
      ? db
...
// GET /sales-orders/:id
// Also resolves payment_mode for the single-order response.
router.get("/sales-orders/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...formatSalesOrder(order, customer?.name, items.length, delivery?.paymentMethod ?? null),
