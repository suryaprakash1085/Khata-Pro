...
router.get("/deliveries/my", requireDriverAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    data: rows.map((r) => ({
      ...
      customer_phone: maskPhone(r.customerPhone),
      customer_has_phone: !!r.customerPhone,
    })),
    total: Number(totalResult[0].count),
    page,
    limit,
...
router.post("/deliveries/:id/payment", requireDriverAuth, async (req, res): Promise<void> => {
  ...
  if (customer) {
    ...
    const [txn] = await db.insert(transactionsTable).values({
      ...
      balanceAfter: currentBalance.toString(), // unchanged — COD doesn't affect khata balance
