...
router.post("/returns", requireAuth, async (req, res): Promise<void> => {
  ...
  const returnAmount = unitPrice * qty;
  const today = new Date().toISOString().split("T")[0];
  // AuthPayload uses `userId`, not `id` — this was the bug causing the 500
  // (created_by ended up undefined, violating the NOT NULL constraint).
  const { userId: createdBy } = (req as any).user as AuthPayload;
