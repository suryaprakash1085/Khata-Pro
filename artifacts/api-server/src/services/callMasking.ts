...
// Docs: https://developer.exotel.com/api/make-a-call-connect-two-parties
//
// ⚠️ TEMPORARY MOCK MODE ⚠️
// Exotel's Connect API is hard-blocked until the account completes KYC
// (403 "Your account is not yet KYC compliant"). Until KYC is done, set
// CALL_MASKING_MOCK=true in .env to skip the real Exotel call and just log
// + return a fake success — so the rest of the app (routes, delivery
// status, frontend handling) can still be tested end-to-end.
// Remove MOCK_MODE (or set it to false) once KYC is approved.
//
// Required env vars:
//   EXOTEL_SID        — your Exotel account SID
//   EXOTEL_API_KEY     — API key (Basic Auth username)
//   EXOTEL_API_TOKEN   — API token (Basic Auth password)
//   EXOTEL_CALLER_ID   — your purchased Exophone (virtual number), e.g. 08047xxxxx
//   CALL_MASKING_MOCK  — "true" to bypass the real Exotel call (pre-KYC testing)

const EXOTEL_SID = process.env.EXOTEL_SID ?? "";
const EXOTEL_API_KEY = process.env.EXOTEL_API_KEY ?? "";
const EXOTEL_API_TOKEN = process.env.EXOTEL_API_TOKEN ?? "";
const EXOTEL_CALLER_ID = process.env.EXOTEL_CALLER_ID ?? "";
const MOCK_MODE = process.env.CALL_MASKING_MOCK === "true";
...
export async function initiateMaskedCall(params: {
  ...
  // ---- MOCK PATH (pre-KYC testing) ----
  if (MOCK_MODE) {
    console.log(
      `[MOCK callMasking] delivery=${deliveryId} would call driver=${driverPhone} -> customer=${customerPhone}`
    );
    return {
      callSid: `mock-${deliveryId}-${Date.now()}`,
