...
// "Unit" is how stock is COUNTED (how many packets/pieces you have).
const UNITS = ['pcs', 'pkt', 'box', 'bottle', 'dozen'];
// "Pack Size" is how much is INSIDE each unit (e.g. 50 g per packet).
// Kept separate from UNITS so "50 g" and "18 pcs" don't collide into one field.
const PACK_SIZE_UNITS = ['g', 'kg', 'ml', 'l'];
...
export default function AddProductScreen() {
  ...
  const [packSize, setPackSize] = useState(''); // e.g. "50" — quantity inside each unit
  const [packUnit, setPackUnit] = useState('g'); // e.g. "g" — measure for packSize
  ...
  useEffect(() => {
    if (isEditMode && existingProduct && !hydrated) {
      // Pack size (e.g. "50 g") is stored as a "(50 g)" suffix on the name,
      // since there's no separate pack_size column on the backend yet.
      // Pull it back out here so it re-populates its own field on edit.
      const rawName = existingProduct.name ?? '';
      const packMatch = rawName.match(/^(.*?)\s*\((\d+(?:\.\d+)?)\s*(g|kg|ml|l)\)\s*$/i);
      if (packMatch) {
        setName(packMatch[1].trim());
        setPackSize(packMatch[2]);
        setPackUnit(packMatch[3].toLowerCase());
      } else {
        setName(rawName);
      }
      ...
      // Older products may have a weight (g/kg/ml/l) saved directly as the
      // counting unit — treat that as "1 <weight>" pack size, counted in pcs.
      const savedUnit = existingProduct.unit ?? 'pcs';
      if ((PACK_SIZE_UNITS as string[]).includes(savedUnit) && !packMatch) {
        setUnit('pcs');
        setPackUnit(savedUnit);
      } else {
        setUnit(savedUnit);
      }
  ...
  const handleSave = () => {
    ...
    // Fold "Pack Size" (e.g. 50 g) into the product name as a "(50 g)" suffix,
    // since the backend only stores one `unit` field today. This is what
    // keeps "Red Chilli Powder 50g" and "18 pcs in stock" from colliding —
    // the pack size travels with the name, the counting unit stays in `unit`.
    const trimmedPackSize = packSize.trim();
    const finalName =
      trimmedPackSize.length > 0 ? `${name.trim()} (${trimmedPackSize} ${packUnit})` : name.trim();
    ...
    <View style={{ flex: 1, backgroundColor: THEME.background }}>
      ...
      >
        ...
        <SectionCard icon="package" number={1} title="Product Information">
          <FieldRow z={60}>
            <FieldCol>
              ...
              />
              {packSize.trim().length > 0 && name.trim().length > 0 ? (
                <Text style={styles.charCount}>
                  Will be saved as: {name.trim()} ({packSize.trim()} {packUnit})
                </Text>
              ) : null}
            </FieldCol>
            <FieldCol>
        ...
        <SectionCard icon="archive" number={4} title="Inventory">
          <FieldRow z={36}>
            <FieldCol>
              <Text style={styles.fieldLabel}>Pack Size (optional)</Text>
              <View style={styles.barcodeRow}>
                <TextInput
                  placeholder="e.g. 50"
                  placeholderTextColor={THEME.placeholder}
                ...
                />
                <View style={[styles.unitChipsRow, { flexShrink: 0 }]}>
              ...
              <Text style={styles.summaryHint}>
                How much is in one {unit} — e.g. 50 g per packet. Leave blank if not applicable.
              </Text>
            </FieldCol>
          </FieldRow>
          ...
          <FieldRow z={35}>
            <FieldCol>
              ...
              <TextInput
                ...
                style={styles.textInput}
              />
              <Text style={styles.summaryHint}>Number of {unit}, not the total weight/volume.</Text>
            </FieldCol>
            <FieldCol>
              <Text style={styles.fieldLabel}>
              ...
              <Text style={styles.summaryHint}>How stock is counted — packets, boxes, bottles etc.</Text>
            </FieldCol>
            <FieldCol>
