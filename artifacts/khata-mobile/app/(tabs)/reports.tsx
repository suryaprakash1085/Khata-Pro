...
type PaymentMode = 'cash' | 'online' | 'cheque' | 'upi' | 'cod';
...
// ---- Online Sales Orders (from delivery-app / sales_orders table) ----
// Mirrors GET /api/sales-orders response shape from artifacts/api-server/src/routes/sales-orders.ts
interface OnlineSalesOrder {
  id: number;
  business_id: number;
  customer_id: number;
  customer_name: string;
  channel: string; // 'online' | 'in_store' etc.
  status: 'pending' | 'invoiced' | 'cancelled' | string;
  amount: number;
  tax: number;
  invoice_no: string | null;
  payment_mode: PaymentMode | null; // resolved server-side from the linked deliveries row
  entry_date: string;
  item_count: number;
  created_at: string;
}
...
interface SalesOrderListResponse {
  data: OnlineSalesOrder[];
  total: number;
  page: number;
  limit: number;
}

function useListSalesOrders(
  params: { business_id?: number; from?: string; to?: string; channel?: string; limit?: number },
  enabled: boolean,
) {
  return useQuery<SalesOrderListResponse>({
    ...
    queryFn: () => {
      ...
      return customFetch<SalesOrderListResponse>(`/api/sales-orders?${search.toString()}`, { responseType: 'json' });
    },
  });
}
...
function useVendorPendingTotal(params: { business_id?: number }, enabled: boolean) {
  return useQuery<{ total_pending: number }>({
    queryKey: ['purchases', 'pending-total', params],
    enabled,
    queryFn: () => {
      const search = new URLSearchParams();
      if (params.business_id) search.set('business_id', String(params.business_id));
      return customFetch<{ total_pending: number }>(`/api/purchases/pending-total?${search.toString()}`, { responseType: 'json' });
    },
  });
}
...
export default function ReportsScreen() {
  ...
  const { data: vendorPendingTotal } = useVendorPendingTotal({ business_id: business?.id }, !!business?.id);
  ...
  const {
    ...
    isError: onlineOrdersError,
  } = useListSalesOrders(
    { business_id: business?.id, from: onlineOrdersDateRange.from, to: onlineOrdersDateRange.to, channel: 'online', limit: 1000 },
    !!business?.id && salesSource === 'online',
  );
...
const paginatedPurchases = useMemo(() => {
  const all = purchasesResponse?.data ?? [];
 ...
 const {
  ...
  // Returns for the SAME period as Product Sales — used only to net qty_sold
  ...
  const returnedQtyByProductName = useMemo(() => {
    const map = new Map<string, number>();
    (returnsForProductSales?.items ?? []).forEach((r) => {
      map.set(r.product_name, (map.get(r.product_name) ?? 0) + r.qty);
    });
    return map;
  }, [returnsForProductSales]);

  // Product Sales data with qty_sold netted down by same-period returns —
  // this is what every downstream view (table, chart, CSV export, top/bottom
  // performers) should read from instead of the raw productSalesData.
  const productSalesNet = useMemo(() => {
    if (!productSalesData) return productSalesData;
    return productSalesData.map((p) => {
      const returnedQty = returnedQtyByProductName.get(p.name) ?? 0;
      return returnedQty > 0 ? { ...p, qty_sold: Math.max(p.qty_sold - returnedQty, 0) } : p;
    });
  }, [productSalesData, returnedQtyByProductName]);
...
const paymentStats = useMemo(() => {
  ...
  // Online sales — Total Orders counts every non-cancelled order placed via the app.
  // Total Revenue/Avg Order Value only count orders that reached "invoiced"
  // (stock deducted, confirmed) — mirrors how a Direct bill only counts once
  // it's actually billed, not just "in progress".
  const onlineSalesReal = useMemo(() => {
    const all = (onlineOrdersResponse?.data ?? []).filter((o) => o.status !== 'cancelled');
    const invoiced = all.filter((o) => o.status === 'invoiced');
    const totalRevenue = invoiced.reduce((sum, o) => sum + (o.amount ?? 0), 0);
    return {
      totalOrders: all.length,
      totalRevenue,
      netSales: totalRevenue,
      avgOrderValue: invoiced.length ? totalRevenue / invoiced.length : 0,
    };
  }, [onlineOrdersResponse]);

  const onlinePaymentBreakdown = useMemo(() => {
    const invoiced = (onlineOrdersResponse?.data ?? []).filter((o) => o.status === 'invoiced');
    const map = new Map<PaymentMode, number>();
    invoiced.forEach((o) => {
      if (!o.payment_mode) return;
      map.set(o.payment_mode, (map.get(o.payment_mode) ?? 0) + o.amount);
    });
...
const {
  ...
  const productSalesFiltered = useMemo(() => {
    ...
    return productSalesNet.filter((p) => (p.category ?? 'Uncategorised') === productSalesCategory);
  }, [productSalesNet, productSalesCategory]);
              ...
              <Card colors={colors} style={{ justifyContent: 'space-between' }}>
                <View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 6, marginBottom: 10 }}>
                    {(['direct', 'online'] as const).map((mode) => {
                      const active = salesSource === mode;
                      return (
                        <Pressable
                          key={mode}
                          onPress={() => setSalesSource(mode)}
                          style={{
                            flex: 1,
                            paddingVertical: 8,
                            borderRadius: 8,
                        ...
                        >
                          <Text style={{ fontFamily: FONT_REGULAR, fontSize: 12.5, color: active ? '#fff' : colors.foreground, fontWeight: active ? '700' : '400' }}>
                            {mode === 'direct' ? 'Direct (Shop)' : 'Online (App)'}
                          </Text>
                        </Pressable>
                      );
                    })}
                  ...
                  {salesSource === 'direct' ? (
                    ...
                    <Text style={{ color: colors.destructive, fontSize: 12, fontFamily: FONT_REGULAR, marginBottom: 6 }}>
                      Couldn't load online orders
                    </Text>
                  ) : (
                    <KpiGrid
                      colors={colors}
                      items={[
                        { label: 'Total Orders', value: String(onlineSalesReal.totalOrders) },
                        { label: 'Total Revenue', value: fmt(onlineSalesReal.totalRevenue) },
                        { label: 'Net Sales', value: fmt(onlineSalesReal.netSales), color: colors.success },
                        { label: 'Avg Order Value', value: fmt(onlineSalesReal.avgOrderValue) },
                      ]}
                    />
                  )}
                </View>
                ...
                <View style={{ marginTop: 6 }}>
                  ...
                  {salesSource === 'direct' ? (
                    ...
                    <ActivityIndicator color={colors.primary} style={{ marginVertical: 6 }} />
                  ) : onlinePaymentBreakdown.length === 0 ? (
                    <Text style={{ fontSize: 11, fontFamily: FONT_REGULAR, color: colors.mutedForeground }}>
                      No confirmed online orders yet
                    </Text>
                  ) : (
                    <KeyValueList
                      colors={colors}
                      rows={onlinePaymentBreakdown.map((p) => ({
                        label: PAYMENT_MODE_LABEL[p.payment_mode] ?? p.payment_mode,
                        value: fmt(p.total_amount),
                      }))}
                    />
                  )}
      ...
      {purchasesLoading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: LAYOUT.cardPadding }}>
          ...
          <View style={{ marginBottom: 12, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: colors.border }}>
            ...
            <Text style={{ fontSize: 20, fontFamily: FONT_BOLD, fontWeight: '700', color: colors.destructive }}>
              {fmt(vendorKpis.pendingPayments)}
            </Text>
          </View>
          ...
          {purchasePageCount > 1 && (
            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 14 }}>
              <Pressable
                onPress={() => setPurchasePage((p) => Math.max(1, p - 1))}
                disabled={purchasePage === 1}
                style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, borderWidth: 1, borderColor: colors.border, opacity: purchasePage === 1 ? 0.4 : 1 }}
              >
                <Feather name="chevron-left" size={14} color={colors.foreground} />
              </Pressable>
              {Array.from({ length: purchasePageCount }, (_, i) => i + 1).map((pg) => (
                <Pressable
                  key={pg}
                  onPress={() => setPurchasePage(pg)}
                  style={{
                    width: 28, height: 28, borderRadius: 6, alignItems: 'center', justifyContent: 'center',
                    backgroundColor: pg === purchasePage ? colors.primary : 'transparent',
                    borderWidth: 1, borderColor: pg === purchasePage ? colors.primary : colors.border,
                  }}
                >
                  <Text style={{ fontSize: 12, fontFamily: FONT_BOLD, color: pg === purchasePage ? '#fff' : colors.foreground }}>{pg}</Text>
                </Pressable>
              ))}
              <Pressable
                onPress={() => setPurchasePage((p) => Math.min(purchasePageCount, p + 1))}
                disabled={purchasePage === purchasePageCount}
                style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, borderWidth: 1, borderColor: colors.border, opacity: purchasePage === purchasePageCount ? 0.4 : 1 }}
              >
                    ...
                    onPress={() =>
                      exportCsv(
                        ...
                        [
                          { key: 'name', label: 'Product' },
                          { key: 'category', label: 'Category' },
                          { key: 'qtySold', label: 'Qty Sold (Net of Returns)' },
