...
// // ---------------------------------------------------------------------------
// // Matches any "PREFIX-1234" style invoice token — covers real invoice
// // numbers like INV-0010 as well as the TXN-<id> fallback label used below
// // for bills whose description didn't parse cleanly. Both bill descriptions
// // and payment descriptions must be matched with the SAME pattern so a
// // quick-pay payment always links back to the bill it was paid against,
// // even when that bill fell back to a TXN-<id> label.
...
//   // Backend already orders by entry_date desc; break ties by id desc so the
//   // most recently created bill for a given day still shows up first.
...
// // `pnpm add jspdf` is required for this import to resolve.
...
// type BillItem = { product_id: number; product_name: string; unit: string; qty: number; unit_price: number };
// type ReturnLine = { product_id: number; product_name: string; unitPrice: number; maxQty: number; returnQty: string; reason: ReturnReason };
...
//   try {
//     const items = await customFetch<BillItem[]>(`/api/transactions/${bill.id}/items`, { responseType: 'json' });
//     setReturnLines(items.map((i) => ({
...
//       invoice_no: returnBill.invoiceNumber,
//       customer_id: returnBill.customerId,
//       customer_name: returnBill.customerName,
//       items: linesToReturn.map((l) => ({
//         product_id: l.product_id,
//         product_name: l.product_name,
//         qty: parseInt(l.returnQty, 10),
//         return_amount: parseInt(l.returnQty, 10) * l.unitPrice,
//         reason: l.reason,
//       })),
//     };
//     await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/returns`, {
//       method: 'POST',
...
//             <Text style={[styles.newBillBtnText, { color: colors.primaryForeground }]}>New Bill</Text>
//           </Pressable>
//         </View>
...
//                       style={[styles.modalActionBtn, { backgroundColor: colors.primary, borderRadius: colors.radius }]}
//                     >
//                       <Feather name="download" size={15} color={colors.primaryForeground} />
//                       <Text style={[styles.modalActionBtnText, { color: colors.primaryForeground }]}>Download Receipt</Text>
//                     </Pressable>
//                   )}
//                 </View> */}
...
//                 {/* <View style={styles.modalActionsRow}>
//   {selectedBill.status !== 'paid' && (
//     <Pressable onPress={() => { const bill = selectedBill; setSelectedBill(null); goPayNow(bill); }} style={[styles.modalActionBtn, { backgroundColor: colors.success, borderRadius: colors.radius }]}>
//       <Feather name="credit-card" size={15} color="#fff" />
//       <Text style={styles.modalActionBtnText}>Pay Now</Text>
//     </Pressable>
//   )}
...
//       </Pressable>
//       <Pressable
//         onPress={() => { const bill = selectedBill; setSelectedBill(null); openReturnModal(bill); }}
...
//                   style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 6, width: 60, textAlign: 'center', paddingVertical: 6, fontFamily: FONT_FAMILY, color: colors.foreground }}
//                 />
...
//                           }}
//                         >
//                           <Text style={{ fontFamily: FONT_FAMILY, fontSize: 11, color: active ? colors.primary : colors.mutedForeground }}>{r}</Text>
//                         </Pressable>
//                       );
//                     })}
//                   </View>
...
//         style={[styles.modalActionBtn, { backgroundColor: '#DC2626', borderRadius: colors.radius, marginTop: 14, opacity: submittingReturn ? 0.6 : 1 }]}
//       >
//         {submittingReturn ? <ActivityIndicator color="#fff" /> : <Text style={styles.modalActionBtnText}>Confirm Return</Text>}
//       </Pressable>
//     </View>
//   </View>
// </Modal>
...
// Place this file at your billing-list route, e.g. app/billing-list.tsx
// (matches the WebSidebar entry: { key: 'Bills', label: 'BillingList', path: '/billing-list' })
//
// NOTE ON DATA SHAPE
// -------------------
// The /transactions API does NOT store a dedicated invoice_number,
// grand_total-per-bill, or status field. Instead, the New Bill screen
// creates:
//   - one `you_gave` transaction per bill, with
//     description = `Invoice ${invoiceNumber}: ${itemSummary}` and
//     amount = the bill's grand total
//   - zero or more `you_got` transactions for payments received, with
//     description containing `for invoice ${invoiceNumber}`
//
// This screen re-derives "bills" by:
//   1. Pulling every `you_gave` transaction as one bill row.
//   2. Pulling every `you_got` transaction, parsing the invoice number
//      out of its description, and summing payments per invoice.
//   3. Comparing paid-so-far vs the bill amount to label it
//      Paid / Partially Paid / Pending.
//
// PAY NOW
// -------
// Tapping "Pay Now" on a Pending / Partially Paid bill navigates to the
// New Bill screen (`/billing`) with these query params:
//   customer_id, customer_name, invoice_number, amount, due_amount
//
// DOWNLOAD RECEIPT (PDF)
// -----------------------
// Shown for Paid / Partially Paid bills. Requires the `jspdf` package:
//   pnpm add jspdf   (run inside artifacts/khata-mobile)
// On web it downloads a real PDF file. On native it falls back to the
// share sheet with plain-text receipt content (no extra native deps).
//
// RETURN
// ------
// Shown for Paid / Partially Paid bills. Opens a separate modal that:
//   1. Fetches the bill's line items via GET /api/transactions/:id/items
//   2. Lets the cashier pick a return qty + reason per product
//   3. Submits POST /api/returns, which the backend uses to restock
//      inventory, refund the customer, and log the return for the
//      Reports > Return Report screen.
//
// IMPORTANT: the Return modal is rendered as a SIBLING of the bill
// detail modal (not nested inside `{selectedBill && (...)}`), because
// opening a return clears `selectedBill` to close the detail modal —
// nesting it inside that fragment would unmount the Return modal too.
...
//   setSubmittingReturn(true);
//   try {
//     // Backend accepts ONE product per POST — loop and submit each returned line separately.
