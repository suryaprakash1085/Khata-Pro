...
export default function DeliveryListScreen() {
  ...
  const openAssignModal = (delivery: Delivery) => {
   ...
   const handleAssign = (driverId: number) => {
    ...
    if (assignDriver.isPending) return;

    assignDriver.mutate(
      { id: selectedDelivery.id, data: { driver_id: driverId } },
      {
        onSuccess: () => {
          console.log('✅ Driver assign SUCCESS — backend creates the assign notification automatically');
...
const handleConfirmOrderSubmit = () => {
  if (!salesOrder) return;
  confirmOrder.mutate(
    { id: salesOrder.id, data: { status: 'invoiced' } },
    {
      onSuccess: () => {
