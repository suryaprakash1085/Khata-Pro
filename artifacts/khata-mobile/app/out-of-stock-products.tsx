...
export default function OutOfStockProductsScreen() {
  ...
  const confirmRestock = () => {
    ...
    const newQty = (restockItem.stock_qty ?? 0) + addQty;

    updateProduct.mutate(
      { id: restockItem.id, data: { stock_qty: newQty } },
      {
        onSuccess: () => {
          ...
          refetch();
          showAlert('Restocked', `${restockItem.name} updated to ${newQty} ${restockItem.unit ?? ''}`.trim());
          setRestockItem(null);
          setRestockQty('');
          setRestockError(null);
        },
        onError: () => {
          setRestockError('Could not update stock. Please try again.');
        },
      }
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={closeRestock}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Restock {restockItem?.name}</Text>
            <Text style={styles.modalSubtitle}>
              Current stock: {restockItem?.stock_qty ?? 0} {restockItem?.unit}
            </Text>

            {restockError ? <Text style={styles.modalError}>{restockError}</Text> : null}
            ...
            <View style={styles.modalButtons}>
              <Pressable style={styles.modalCancelBtn} onPress={closeRestock} disabled={updateProduct.isPending}>
                <Text style={styles.modalCancelText}>Cancel</Text>
              </Pressable>
              <Pressable
                style={[styles.modalConfirmBtn, updateProduct.isPending && { opacity: 0.6 }]}
...
const styles = StyleSheet.create({
  ...
  modalCard: { width: '100%', maxWidth: 380, backgroundColor: '#FFFFFF', borderRadius: 12, padding: 20 },
  modalTitle: { fontSize: 16, fontWeight: '700', color: '#0F172A', fontFamily: 'Inter_700Bold' },
  modalSubtitle: { fontSize: 13, color: '#64748B', marginTop: 4, fontFamily: 'Inter_500Medium' },
  modalError: { fontSize: 12, color: '#DC2626', marginTop: 10, fontFamily: 'Inter_500Medium' },
  modalInput: { marginTop: 14, borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, paddingHorizontal: 12, height: 44, fontSize: 14, color: '#0F172A', fontFamily: 'Inter_400Regular' },
  ...
  modalCancelText: { fontSize: 13, fontWeight: '600', color: '#334155', fontFamily: 'Inter_600SemiBold' },
  modalConfirmBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8, backgroundColor: '#7C3AED', minWidth: 130, alignItems: 'center', justifyContent: 'center' },
  modalConfirmText: { fontSize: 13, fontWeight: '600', color: '#FFFFFF', fontFamily: 'Inter_600SemiBold' },
});