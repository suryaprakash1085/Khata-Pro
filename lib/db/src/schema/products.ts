...
import { pgTable, bigserial, bigint, varchar, text, boolean, timestamp, decimal, integer, pgEnum, uniqueIndex } from "drizzle-orm/pg-core";
...
export const productsTable = pgTable(
  ...
  (table) => ({
    // Enforces one barcode per business at the database level — the real
    // guarantee against duplicates (frontend checks are just UX). Partial
    // index: ignores soft-deleted rows and NULL barcodes, since a product
    // can be saved without a barcode.
    businessBarcodeUnique: uniqueIndex("products_business_barcode_unique")
      .on(table.businessId, table.barcode)
      .where(sql`${table.barcode} IS NOT NULL AND ${table.isDeleted} = false`),
  }),
);
