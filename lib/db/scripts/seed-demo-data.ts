...
async function main() {
  ...
  const planMatrix: Array<{ plan: "free" | "pro" | "premium"; status: "active" | "expired" | "cancelled" }> = [
  ...
  let custIdx = 0;
  const allCustomersByBusiness: Record<number, any[]> = {};
  for (const biz of businesses) {
    ...
    for (let i = 0; i < custCount; i++) {
      ...
      const [customer] = await db.insert(customersTable).values({
        ...
        openingBalance,
        openingBalanceType: openingType as "credit" | "debit",
  ...
  for (const biz of businesses) {
    ...
    for (const cust of custs) {
      ...
      for (let t = 0; t < txnCount; t++) {
        const type: "you_gave" | "you_got" = Math.random() > 0.5 ? "you_gave" : "you_got";
...
function eqPhone(phone: string) {
  return eq(usersTable.phone, phone);
}
function eqId(table: typeof customersTable, id: number) {
