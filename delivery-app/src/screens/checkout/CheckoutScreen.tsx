...
// Address already includes latitude?/longitude? — no local redeclaration needed.
import { deliveryFeeService, DeliveryFeeCalculation } from '../../services/deliveryFeeService';
// ⚠️ CONFIRM THIS PATH — must be the same base URL constant that
// deliveryFeeService.ts uses internally to hit the backend. If your
// project's constant lives somewhere else (e.g. '../../config', or a
// named export like `BASE_URL`), update this import to match.
...
export default function CheckoutScreen({ navigation, route }: any) {
  ...
  // 👇 live delivery fee calculation state
  ...
  // 👇 fetch the live delivery fee once we have a geocoded address + cart items
  useEffect(() => {
    const businessId = localCartItems[0]?.restaurantId;
    // restaurantId here is expected to already be the numeric Khata-Pro
    // business_id — same assumption CreateOrderData.restaurantId makes
    // elsewhere in this app. If restaurants/shops aren't 1:1 with
    // business_id in your data model yet, that mapping needs to happen
    // before this call, not inside CheckoutScreen.
    if (!selectedAddress?.latitude || !selectedAddress?.longitude || !businessId) {
    ...
    deliveryFeeService
      ...
      .then((result) => {
        if (!cancelled) setDeliveryCalc(result);
      })
      .catch((err) => {
        if (!cancelled) {
          setDeliveryCalc(null);
          setDeliveryFeeError(
            err?.error || err?.message || 'Could not calculate delivery fee.'
          );
        }
      })
      .finally(() => {
  ...
  }, [selectedAddress, localCartItems]);
  ...
  const billingData = useMemo(() => {
    ...
    // 👇 no more hardcoded delivery fee — the number comes only from
    // what the backend returned via deliveryFeeService.calculate().
    const deliveryFee = hasItems ? (deliveryCalc?.delivery_fee ?? 0) : 0;
    ...
    return {
      ...
      total: hasItems ? itemTotal + deliveryFee + 7 + Math.round(itemTotal * 0.05) : 0,
    };
  }, [localCartItems, calculateItemTotal, deliveryCalc]);
  ...
  // 👇 SINGLE handlePlaceOrder — now actually persists the order to the
  // backend (`POST /public/sales-orders`) before touching local state.
  // This is what lets the delivery-fee/tax snapshot reach `sales_orders`
  // and flow through to the driver's Order Details screen.
  const handlePlaceOrder = async () => {
    ...
    if (deliveryFeeLoading) {
      Alert.alert('Please wait', 'Still calculating your delivery fee.');
      return;
    }

    if (deliveryFeeError) {
      Alert.alert('Delivery Fee Error', deliveryFeeError);
      return;
    }
    ...
    if (!deliveryCalc) {
      Alert.alert(
        ...
        'We could not verify the delivery fee for this address. Please try selecting your address again.'
      );
      return;
    }
    ...
    try {
      ...
      // apiClient.post returns the raw Axios response — the created
      // sales-order fields (id, delivery_fee, etc.) live under `.data`,
      // not directly on the response object. This differs from
      // deliveryFeeService.calculate(), whose backend/wrapper returns
      // the payload unwrapped — confirm that difference against your
      // apiClient's interceptor if this still doesn't match at runtime.
      const createdOrder = await apiClient.post('/public/sales-orders', {
        business_id: Number(businessId),
        customer_id: user.id, // ⚠️ confirm this matches customers.id, not users.id
        channel: 'online',
        tax: tax,
        shipping_address: getAddressDisplay(),
        customer_latitude: selectedAddress.latitude,
        customer_longitude: selectedAddress.longitude,
        payment_method: selectedPayment === 'cash' ? 'cod' : 'online',
        items: localCartItems.map((item: any) => ({
          product_id: item.id, // ⚠️ confirm this matches products.id
          qty: typeof item.quantity === 'number' ? item.quantity : parseInt(item.quantity) || 1,
          unit_price: typeof item.price === 'number' ? item.price : parseFloat(item.price) || 0,
        })),
      });

      const order = {
        id: `ORD${createdOrder.data.id}`,
      ...
      Alert.alert('Order Failed', err.message || 'Something went wrong placing your order.');
    } finally {
          ...
          <View style={styles.billContainer}>
            ...
            <View style={styles.billRow}>
              <Text style={styles.billLabel}>
                Delivery Fee{deliveryCalc ? ` (${deliveryCalc.distance_km} KM)` : ''}
              </Text>
              {deliveryCalc?.is_free_delivery ? (
                <Text style={[styles.billValue, { color: colors.success }]}>FREE</Text>
              ) : (
                <Text style={styles.billValue}>
                  {deliveryFeeLoading ? 'Calculating...' : `₹${deliveryFee}`}
                </Text>
              )}
            </View>
            <View style={styles.billRow}>
