...
export default function CheckoutScreen({ navigation, route }: any) {
  ...
  const handlePlaceOrder = () => {
    ...
    const order = {
      ...
      deliveryAddress: selectedAddress,
      paymentMethod: selectedPayment,
      // 👇 delivery-fee snapshot — backend still recalculates authoritatively
      // (see SALES_ORDERS_INTEGRATION_NOTE.md), this just carries the
      // numbers along in case orderService needs them for the payload.
      deliveryDistanceKm: deliveryCalc.distance_km,
      deliveryFee: deliveryCalc.delivery_fee,
      customerLatitude: selectedAddress.latitude,
      customerLongitude: selectedAddress.longitude,
    };

    // ⚠️ CALL-OUT: this still only calls the local addOrder() from
    // OrderContext + clears the cart locally — it never calls
    // orderService.createOrder(order). If orders need to persist to the
    // backend (they do, for the delivery-fee snapshot above to reach
    // sales_orders), wire that call in here once it's clear whether
    // addOrder() already does this server-side internally.
    addOrder(order);
    clearCart();
    setLocalCartItems([]);
    setIsOrderPlaced(true);
