...
// // //         'Order',
// // //       tax: finalTax,
// // //       // 👇 FIX: map local selectedMethod ('razorpay'/'cash') to the
...
// //   // 👇 GST-a discount apply pannina apram, discounted subtotal mela calculate pannunga
// //   //    (Cart screen la exact same logic use pannirukanga, adha match pannunga)
// //   const discountedSubtotal = finalSubtotal - finalDiscount;
// //   const finalTax = tax > 0 ? tax : calculateTax(discountedSubtotal);

// //   const effectiveGstRate = discountedSubtotal > 0 ? Math.round((finalTax / discountedSubtotal) * 100) : 0;
...
// //         'Order',
// //       tax: finalTax,
// //       discount: finalDiscount, // 👈 NEW: Include discount in payload
// //       promo_code: promoCode, // 👈 NEW: Include promo code
// //       // 👇 FIX: map local selectedMethod ('razorpay'/'cash') to the
// //       // backend's payment_method enum ('online'/'cod')
// //       payment_method: selectedMethod === 'cash' ? 'cod' : 'online',
// //       items: formattedItems,
...
//   // ==========================================================
//   // CALCULATE TAX - DYNAMIC BASED ON ITEM GST RATES
//   // Always computed off the amount actually passed in (post-discount),
//   // never trusted blindly from route params.
//   // ==========================================================
//   const calculateTax = (amountBase: number) => {
//     if (!cartItems || cartItems.length === 0) return 0;
//     const total = cartItems.reduce((sum: number, item: any) => {
//       const itemTotal = (item.price || 0) * (item.quantity || 1);
//       const gstRate = item.gst_rate || 0;
...
//   // ==========================================================
//   // ✅ FIXED CALCULATIONS
//   // Root cause of both bugs: this screen was trusting `tax` and
//   // `totalAmount` route params as-is. Those params can carry STALE
//   // values from the Cart screen (calculated before the discount fix),
//   // so "GST 10%" and "Total ₹975" kept showing even after Cart itself
//   // was showing the correct ₹503 breakdown.
//   //
//   // Fix: subtotal/deliveryFee/discount are still taken from route
//   // params (those are fine, not derived values), but tax and the
//   // final total are ALWAYS recalculated here from the discounted
//   // subtotal — never taken from a possibly-stale param.
//   // ==========================================================
//   const finalSubtotal = subtotal > 0 ? subtotal : totalAmount - deliveryFee - tax;
//   const finalDeliveryFee = deliveryFee > 0 ? deliveryFee : 40;
//   const finalDiscount = discount > 0 ? discount : 0;

//   // GST must be calculated on the subtotal AFTER discount is applied —
//   // same logic as CartScreen's calculateTotal(), kept in sync with it.
//   const discountedSubtotal = finalSubtotal - finalDiscount;

//   // 👇 FIX #1 (GST 10% bug): don't use `tax > 0 ? tax : ...` anymore.
//   // The stale `tax` param (e.g. ₹45, calculated pre-discount) was
//   // always > 0, so calculateTax() never actually ran. Recalculate
//   // fresh, every time, off the discounted subtotal.
//   const finalTax = calculateTax(discountedSubtotal);
...
//   // 👇 FIX #2 (stale ₹975 Total bug): don't fall back to route param
//   // `totalAmount` anymore — it can be stale (calculated before the
//   // discount fix on Cart). Always use the freshly calculated total,
//   // which now matches the Cart screen's Order Summary exactly.
...
const PaymentScreen: React.FC<PaymentScreenProps> = ({
  ...
  const calculateTax = (discountAmount: number) => {
    ...
    // 👇 REAL FIX: the old version took a parameter but never used it —
    // it always taxed the full item price. GST must be applied on the
    // DISCOUNTED item amount, same as CartScreen's calculateTotal():
    // work out what fraction of the raw subtotal the discount removes,
    // then apply that same fraction to every item before taxing it.
  ...
  // 👇 FIX #1 (GST 10% / ₹45 bug): don't use `tax > 0 ? tax : ...`
  // anymore — the stale `tax` param was always > 0 so calculateTax()
  // never ran. ALSO: calculateTax() itself had a hidden bug — it took
  // a parameter but never used it, so it always taxed the FULL item
  // price. Fixed above to actually apply the discount ratio per item.
  // Pass the discount AMOUNT (not the already-discounted subtotal) —
  // calculateTax works out the ratio internally.
