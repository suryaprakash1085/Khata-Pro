...
//   // 👇 NEW — returns cart items eligible for a given promotion
//   const getEligibleCartItems = (promo: Promotion) => {
//     if (!promo.apply_to || promo.apply_to === 'all') {
//       return storeCartItems;
//     }
//     if (promo.apply_to === 'selected') {
//       if (!promo.product_ids || promo.product_ids.length === 0) return [];
//       return storeCartItems.filter((i) => promo.product_ids.includes(Number(i.id)));
//     }
//     if ((promo as any).apply_to === 'category') {
//       const promoCategory = (promo as any).category;
//       if (!promoCategory) return [];
//       return storeCartItems.filter((i: any) => i.category === promoCategory);
//     }
//     return [];
//   };

//   // 👇 NEW — a promotion is worth showing only if it applies to something in the cart
//   const isPromotionApplicable = (promo: Promotion) => getEligibleCartItems(promo).length > 0;
...
//     const eligibleItems = getEligibleCartItems(appliedPromo);
...
//     // 👇 NEW — code correct-ஆ இருந்தாலும், cart-ல applicable product இல்லனா reject பண்ணுங்க
//     if (!isPromotionApplicable(match)) {
//       setPromoError('This promo code is not applicable to the items in your cart');
//       setAppliedPromo(null);
//       return;
//     }
...
//   const applicablePromotions = promotions.filter(isPromotionApplicable); // 👈 NEW
...
//         {(loadingPromotions || promotionsError || applicablePromotions.length > 0) && (
//           <View style={styles.recommendedContainer}>
//             <View style={styles.recommendedHeader}>
...
// >
//   {applicablePromotions.map((item: Promotion) => (
//     <View
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          ...
          <View style={styles.modalSummaryContainer}>
            ...
            {/* 👇 single combined GST line — no more splitting by GST rate */}
            <View style={styles.modalSummaryRow}>
              <Text style={styles.modalSummaryLabel}>GST</Text>
              <Text style={styles.modalSummaryValue}>₹{tax}</Text>
            </View>
...
const CartScreen: React.FC<CartScreenProps> = ({ navigation }) => {
  ...
  const [promoError, setPromoError] = useState<string | null>(null);
  const [promoSuccess, setPromoSuccess] = useState<string | null>(null);
  // 👇 which promotion card is currently selected (radio), before "Apply" is pressed
  const [selectedPromoCard, setSelectedPromoCard] = useState<Promotion | null>(null);
  ...
  // 👇 a promotion is worth showing only if it applies to something in the cart
  const isPromotionApplicable = (promo: Promotion) => getEligibleCartItems(promo).length > 0;
  ...
  const handleApplyPromo = () => {
    ...
    // 👇 code correct-ஆ இருந்தாலும், cart-ல applicable product இல்லனா reject பண்ணுங்க
    if (!isPromotionApplicable(match)) {
      setPromoError('This promo code is not applicable to the items in your cart');
      setAppliedPromo(null);
      return;
  ...
  // 👇 tapping a card just selects it (radio-style), does NOT apply yet
  const handleSelectPromotionCard = (promo: Promotion) => {
    setPromoError(null);
    setPromoSuccess(null);
    // tapping the already-selected card again deselects it
  ...
  // 👇 the single shared "Apply" button uses whichever card is selected
  const handleApplySelectedCard = () => {
    if (!selectedPromoCard) {
      setPromoError('Please select an offer first');
      return;
  ...
  // 👇 FIX: when adding a recommended product to the cart, use the CURRENT
  // store's restaurantId (not the product's own id) so it lands in the SAME
  // store-scoped cart bucket as the rest of the items. This ensures GST from
  // this new item is combined with everything else in calculateTotal().
  const handleAddToCart = (product: any) => {
    const currentStoreId = getStoreId();
    ...
    const cartItem = {
      ...
      quantity: 1,
      image: product.image || 'https://placehold.co/150x150',
      restaurantId: currentStoreId ?? product.id, // 👈 keep it in the same store bucket
      restaurantName: selectedBusiness?.name || product.name,
      gst_rate: product.gst_rate || 0,
      category: product.category,
    };

    const restaurantData = {
      id: currentStoreId ?? product.id,
      name: selectedBusiness?.name || product.name,
      rating: 4.5,
      deliveryTime: 'In Stock',
      cuisine: product.category || 'General',
      image: product.image || 'https://placehold.co/150x150',
