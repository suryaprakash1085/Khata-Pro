...
//   // ✅ Get store ID from SelectedBusinessContext or from cart items
//   const getStoreId = (): number | null => {
//     // First try from selectedBusiness context
//     if (selectedBusiness?.id) {
...
//   // ✅ Handle Cash on Delivery - UPDATED with business_id
...
//         })),
//         description: `Order from ${selectedBusiness?.name || cartItems[0]?.restaurantName || 'Store'}`,
...
//         restaurantName: selectedBusiness?.name || cartItems[0]?.restaurantName || 'QuickBite',
//         items: cartItems.map(item => ({
//           name: item.name,
//           quantity: item.quantity,
//           price: item.price,
...
//   // ✅ Handle Razorpay Payment - UPDATED with business_id
//   const handleRazorpayPayment = async () => {
//     if (cartItems.length === 0) {
//       Alert.alert('Cart is Empty', 'Please add items to your cart first.');
//       return;
...
//     try {
//       // ✅ Create order first with business_id
//       const orderData = {
//         business_id: businessId,
//         customer_id: 1,
//         items: cartItems.map(item => ({
...
//   // ✅ Handle Proceed to Checkout - UPDATED with business_id
//   const handleProceedToCheckout = () => {
//     if (cartItems.length === 0) {
//       Alert.alert('Cart is Empty', 'Please add items to your cart first.');
//       return;
...
//       restaurantName: selectedBusiness?.name || cartItems[0]?.restaurantName || 'QuickBite',
//       cartItems: cartItems,
//       business_id: businessId, // ✅ Pass business_id to AddressSelection
//     });
//   };
...
const CartScreen: React.FC<CartScreenProps> = ({ navigation }) => {
  const { cartItems, updateQuantity, removeFromCart, getTotalPrice, getTotalItems, clearCart, addToCart } =
    useContext(CartContext);
  ...
  // Priority: SelectedBusinessContext (set by HomeScreen/ProductListScreen taps)
  // Fallback: the restaurantId tagged on the first cart item (ProductListScreen
  // now tags this with the STORE id, not the product id).
  const getStoreId = (): number | null => {
    if (selectedBusiness?.id) {
      return Number(selectedBusiness.id);
    }
  ...
  const isItemInCart = (productId: string) => cartItems.some((item) => item.id === String(productId));
  ...
  // ✅ Cash on Delivery — creates the order with the resolved business_id
  const handleCashOnDelivery = async () => {
    if (cartItems.length === 0) {
      Alert.alert('Cart is Empty', 'Please add items to your cart first.');
      return;
    ...
    try {
      ...
      const orderData = {
        business_id: businessId,
        customer_id: 1, // TODO: pull from auth context
        items: cartItems.map((item) => ({
          product_id: parseInt(item.id, 10),
          qty: item.quantity,
          unit_price: item.price,
  ...
  // ✅ Razorpay — also creates the order first with the resolved business_id
  const handleRazorpayPayment = async () => {
    if (cartItems.length === 0) {
      Alert.alert('Cart is Empty', 'Please add items to your cart first.');
      return;
    ...
    if (!isRazorpayReady) {
      Alert.alert(
        'Payment Error',
        'Razorpay is not ready yet. Please wait a moment and try again, or use Cash on Delivery.',
        [{ text: 'OK' }, { text: 'Use Cash on Delivery', onPress: handleCashOnDelivery }],
      );
      return;
    }
  ...
  const renderCartItem = ({ item }: { item: any }) => (
    <View style={styles.cartItem}>
      <Image source={{ uri: item.image || 'https://placehold.co/60x60' }} style={styles.itemImage} />
      <View style={styles.itemInfo}>
        <Text style={styles.itemName}>{item.name}</Text>
        <Text style={styles.itemPrice}>₹{item.price}</Text>
        <Text style={styles.itemRestaurant}>{item.restaurantName}</Text>
  ...
  const renderRecommendedItem = ({ item }: { item: any }) => {
    ...
    return (
      <View style={styles.recommendedItem}>
        <Image source={{ uri: imageUrl }} style={styles.recommendedImage} resizeMode="cover" />
        <View style={styles.recommendedInfo}>
          <Text style={styles.recommendedName} numberOfLines={1}>
            {item.name || 'Unnamed Product'}
          </Text>
          ...
          <Text style={styles.recommendedPrice}>₹{item.selling_price || item.price || 0}</Text>
        </View>
        {inCart ? (
          <View style={styles.quantityContainerSmall}>
            <TouchableOpacity
              style={styles.quantityButtonSmall}
              onPress={() => handleUpdateQuantity({ id: item.id, restaurantId: item.id }, quantity - 1)}
            >
              <Icon name="remove" size={12} color="#fc8019" />
            </TouchableOpacity>
            <Text style={styles.quantityTextSmall}>{quantity}</Text>
          ...
          <TouchableOpacity style={styles.addButtonSmall} onPress={() => handleAddToCart(item)}>
            <Text style={styles.addButtonTextSmall}>+ Add</Text>
          </TouchableOpacity>
        )}
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.cartItemsContainer}>
          {cartItems.map((item, index) => (
            <View key={`${item.id}-${item.restaurantId}-${index}`}>{renderCartItem({ item })}</View>
          ))}
        </View>

        <TouchableOpacity style={styles.viewMoreButton} onPress={navigateToProductList} activeOpacity={0.8}>
          <Icon name="add-circle-outline" size={20} color="#fc8019" />
          <Text style={styles.viewMoreText}>Add More Products</Text>
          <Icon name="arrow-forward" size={16} color="#fc8019" />
        </TouchableOpacity>
      ...
      <View style={styles.checkoutContainer}>
        <TouchableOpacity style={styles.checkoutLeft} onPress={() => setShowSummaryModal(true)} activeOpacity={0.7}>
          <Text style={styles.checkoutTotal}>₹{total}</Text>
          <Text style={styles.checkoutItems}>{getTotalItems()} items</Text>
        </TouchableOpacity>

        <View style={styles.checkoutRight}>
          <TouchableOpacity style={styles.billDetailsButton} onPress={() => setShowSummaryModal(true)}>
            <Icon name="receipt-outline" size={18} color="#fc8019" />
            <Text style={styles.billDetailsText}>Bill Details</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.checkoutButton} onPress={handleProceedToCheckout} disabled={isProcessing}>
            <Text style={styles.checkoutButtonText}>{isProcessing ? 'Processing...' : 'Proceed →'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: '600', color: '#282c3f' },
  headerRight: { width: 40 },
  clearText: { color: '#dc3545', fontSize: 14, fontWeight: '500' },
  ...
  itemImage: { width: 60, height: 60, borderRadius: 8, backgroundColor: '#f0f0f5' },
  itemInfo: { flex: 1, marginLeft: 12 },
  itemName: { fontSize: 14, fontWeight: '500', color: '#282c3f' },
  itemPrice: { fontSize: 16, fontWeight: '600', color: '#fc8019', marginTop: 2 },
  itemRestaurant: { fontSize: 12, color: '#7e808c', marginTop: 2 },
  quantityContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    borderWidth: 1,
  ...
  quantityText: { fontSize: 14, fontWeight: '600', color: '#282c3f', minWidth: 20, textAlign: 'center' },
  ...
  viewMoreText: { fontSize: 15, fontWeight: '600', color: '#fc8019', marginHorizontal: 8 },
  recommendedContainer: {
    marginHorizontal: 16,
    marginBottom: 16,
    backgroundColor: '#ffffff',
  ...
  recommendedHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 12, paddingBottom: 12 },
  recommendedHeaderLeft: { flexDirection: 'row', alignItems: 'center' },
  recommendedTitle: { fontSize: 16, fontWeight: '600', color: '#282c3f', marginLeft: 4 },
  recommendedList: { paddingHorizontal: 12, paddingBottom: 4 },
  recommendedItem: { width: 140, backgroundColor: '#ffffff', borderRadius: 12, borderWidth: 1, borderColor: '#f0f0f5', padding: 8, alignItems: 'center' },
  recommendedImage: { width: 100, height: 100, borderRadius: 8, backgroundColor: '#f8f9fa', marginBottom: 8 },
  recommendedInfo: { alignItems: 'center', width: '100%', marginBottom: 8 },
  recommendedName: { fontSize: 13, fontWeight: '500', color: '#282c3f', textAlign: 'center' },
  recommendedCategory: { fontSize: 11, color: '#7e808c', textAlign: 'center', marginTop: 2 },
  recommendedPrice: { fontSize: 15, fontWeight: '600', color: '#fc8019', marginTop: 4 },
  quantityContainerSmall: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#fc8019', borderRadius: 6, paddingHorizontal: 4, backgroundColor: '#ffffff' },
  quantityButtonSmall: { width: 22, height: 22, justifyContent: 'center', alignItems: 'center' },
  quantityTextSmall: { fontSize: 12, fontWeight: '600', color: '#282c3f', minWidth: 16, textAlign: 'center' },
  addButtonSmall: { borderWidth: 1, borderColor: '#fc8019', paddingHorizontal: 14, paddingVertical: 4, borderRadius: 6, backgroundColor: '#ffffff' },
  addButtonTextSmall: { color: '#fc8019', fontSize: 12, fontWeight: '500' },
  loadingContainer: { paddingVertical: 30, alignItems: 'center' },
  loadingText: { marginTop: 8, fontSize: 14, color: '#7e808c' },
  ...
  processingContainer: { backgroundColor: '#ffffff', borderRadius: 12, padding: 30, alignItems: 'center', width: '80%' },
  processingText: { fontSize: 18, fontWeight: '600', color: '#282c3f', marginTop: 16 },
  processingSubText: { fontSize: 14, color: '#7e808c', marginTop: 8 },
  successOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
  successContainer: { backgroundColor: '#ffffff', borderRadius: 24, padding: 30, width: '90%', maxWidth: 400, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 5 },
  successIconContainer: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#e8f5e9', justifyContent: 'center', alignItems: 'center', marginBottom: 16 },
  successTitle: { fontSize: 24, fontWeight: 'bold', color: '#282c3f', marginBottom: 8 },
  successSubtitle: { fontSize: 14, color: '#7e808c', marginBottom: 20, textAlign: 'center' },
  successDetails: { width: '100%', backgroundColor: '#f8f9fa', borderRadius: 12, padding: 16, marginBottom: 20 },
  successRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#e9ecef' },
  successLabel: { fontSize: 14, color: '#7e808c' },
  successValue: { fontSize: 14, color: '#282c3f', fontWeight: '500' },
  successTotal: { color: '#fc8019', fontWeight: '700', fontSize: 16 },
  successStatus: { color: '#28a745', fontWeight: '600' },
  successButton: { backgroundColor: '#fc8019', borderRadius: 12, paddingVertical: 14, paddingHorizontal: 40, width: '100%', alignItems: 'center', marginBottom: 10 },
  successButtonText: { color: '#ffffff', fontSize: 16, fontWeight: '600' },
  successButtonSecondary: { borderRadius: 12, paddingVertical: 12, paddingHorizontal: 40, width: '100%', alignItems: 'center', borderWidth: 1, borderColor: '#fc8019' },
  successButtonSecondaryText: { color: '#fc8019', fontSize: 16, fontWeight: '500' },
  checkoutContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1, borderTopColor: '#f0f0f5', backgroundColor: '#ffffff' },
  checkoutLeft: { flexDirection: 'column', paddingVertical: 4, paddingRight: 8 },
  checkoutTotal: { fontSize: 20, fontWeight: '700', color: '#fc8019' },
  checkoutItems: { fontSize: 12, color: '#7e808c', marginTop: 1 },
  checkoutRight: { flexDirection: 'row', alignItems: 'center', flex: 1, justifyContent: 'flex-end' },
  billDetailsButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#ffffff', borderWidth: 1.5, borderColor: '#fc8019', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, marginRight: 10 },
  billDetailsText: { color: '#fc8019', fontSize: 13, fontWeight: '600', marginLeft: 6 },
  checkoutButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fc8019', paddingHorizontal: 22, paddingVertical: 12, borderRadius: 8, elevation: 2, shadowColor: '#fc8019', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4 },
  checkoutButtonText: { color: '#ffffff', fontSize: 14, fontWeight: '600' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 },
  emptyText: { fontSize: 20, fontWeight: '600', color: '#282c3f', marginTop: 16 },
  emptySubText: { fontSize: 14, color: '#7e808c', marginTop: 8, textAlign: 'center' },
  shopButton: { marginTop: 24, backgroundColor: '#fc8019', paddingHorizontal: 32, paddingVertical: 12, borderRadius: 8 },
  shopButtonText: { color: '#ffffff', fontSize: 16, fontWeight: '600' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalContainer: { backgroundColor: '#ffffff', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 20, paddingBottom: 30, maxHeight: '80%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f5' },
  modalTitle: { fontSize: 20, fontWeight: '700', color: '#282c3f' },
  modalCloseButton: { padding: 4 },
  modalSummaryContainer: { marginBottom: 16 },
  modalSummaryRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6 },
  modalSummaryLabel: { fontSize: 14, color: '#7e808c' },
  modalSummaryValue: { fontSize: 14, color: '#282c3f' },
  modalTotalRow: { borderTopWidth: 1, borderTopColor: '#e9ecef', paddingTop: 8, marginTop: 4 },
  modalTotalLabel: { fontSize: 16, fontWeight: '600', color: '#282c3f' },
  modalTotalValue: { fontSize: 16, fontWeight: '700', color: '#fc8019' },
  modalCloseButtonFull: { backgroundColor: '#fc8019', borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  modalCloseButtonText: { color: '#ffffff', fontSize: 16, fontWeight: '600' },
});
